<?php
function psAddWAArenderBlog_categoryTies($dat){
	...
	if(!$sqlite){
		...
		$needsReset=true;
			...
			if(!OBSANE($dat,'article')){
				...
				if(OBSANE($dat, 'article')){
					if(($res=RMF_TBL_WAArenderBlog_articles_SQLITE_RESOLVE($sqlite,$dat['article'])) !== 'empty'){	
						if($res !== ''){
							return 'a resolve went wrong '.$res;
						}
						if(!RMF_TBL_WAArenderBlog_articles_FILTERVALIDATE($dat['article'])){
			...
			if(!OBSANE($dat,'category')){
				...
				if(OBSANE($dat, 'category')){
					if(($res=RMF_TBL_WAArenderBlog_categories_SQLITE_RESOLVE($sqlite,$dat['category'])) !== 'empty'){	
						if($res !== ''){
							return 'a resolve went wrong '.$res;
						}
						if(!RMF_TBL_WAArenderBlog_categories_FILTERVALIDATE($dat['category'])){
...
function psEditWAArenderBlog_categoryTies($dat){
	...
	$resResult=RMF_TBL_WAArenderBlog_categoryTies_SQLITE_RESOLVE($sqlite, $OGvalue);
	...
	$ogPk=$OGvalue['pk'];
			...
			if(OBSANE($dat, 'article')){
				$res=RMF_TBL_WAArenderBlog_articles_SQLITE_RESOLVE($sqlite, $dat['article']);
			...
			if(OBSANE($dat, 'category')){
				$res=RMF_TBL_WAArenderBlog_categories_SQLITE_RESOLVE($sqlite, $dat['category']);
...
function psDropWAArenderBlog_categoryTies($dat){
	...
	if(!$sqlite){
		...
		$res=RMF_TBL_WAArenderBlog_categoryTies_SQLITE_RESOLVE($sqlite, $dat);
...
function psGetWAArenderBlog_categoryTies($dat){
	...
	$resResult=RMF_TBL_WAArenderBlog_categoryTies_SQLITE_RESOLVE($sqlite, $dat);
	if($resResult !== ''){
...
function psSearchWAArenderBlog_categoryTies($dat){
	...
	foreach($ret as &$finalReturn)
	{
			...
			if($finalReturn['article']){
				...
				RMF_TBL_WAArenderBlog_articles_SQLITE_RESOLVE($sqlite, $finalReturn['article']);
			}
		
			if($finalReturn['category']){
				$finalReturn['category']=['pk'=>$finalReturn['category']];
				
				RMF_TBL_WAArenderBlog_categories_SQLITE_RESOLVE($sqlite, $finalReturn['category']);
			}
