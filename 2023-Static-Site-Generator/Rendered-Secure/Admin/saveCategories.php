<?php 
	...
	require_once("$_SERVER[DOCUMENT_ROOT]/../-redacted-/blogs/news/RMF/-redacted-_categories/lib.php");
	require_once("$_SERVER[DOCUMENT_ROOT]/../-redacted-/blogs/news/RMF/-redacted-_categories/validation.php");
	...
	foreach($list as $l)
	{
		...
		if(isset($l['pk'])){
			...
			if($checkData['slug'] !== $l['slug']){
				rename("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$checkData[slug]", "$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$l[slug]");
			}
			...
			if($l['shareImage']['slug'] !== $checkData['shareImage']['slug'] && $checkData['shareImage']['ext']){
				@rename(
					"$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$l[slug]/".$checkData['shareImage']['slug'].'.'.$checkData['shareImage']['ext'],
					"$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$l[slug]/".$l['shareImage']['slug'].'.'.$checkData['shareImage']['ext']
				);
			}
			...
			$res=psEdit_redacted__categories($l);
			...
			$l['shareImage']=json_encode($l['shareImage']);
			$res=psAdd_redacted__categories($l);
			if($res !== "SUCCESS"){
	...
	if(is_dir("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/")){
		$sd=scandir("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/");
		$sd=array_filter($sd,function($s){
			return array_search($s,['.','..']) === false && is_dir("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$s");
		});
		foreach($sd as $s)
		{
			if(array_search($s, $slugList) === false){
				DeleteDirectory("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$s");
				$res=psDrop_redacted__categories(['slug'=>$s]);
			}
		}
	}
	//$catTemp=file_get_contents("$_SERVER[DOCUMENT_ROOT]/../-redacted-/blogs/news/category.php.temp");
	
	foreach($slugList as $sl)
	{
		EnsureDirectory("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$sl");

		file_put_contents("$_SERVER[DOCUMENT_ROOT]/../public_html/news/c/$sl/index.php", "<?php require_once(\"$_SERVER[DOCUMENT_ROOT]/../-redacted-/blogs/news/category.php.temp\"); ?>");
	}
