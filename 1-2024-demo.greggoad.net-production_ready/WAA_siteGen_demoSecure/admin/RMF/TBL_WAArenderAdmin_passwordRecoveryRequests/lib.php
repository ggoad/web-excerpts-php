<?php
function psAddWAArenderAdmin_passwordRecoveryRequests($dat){
	...
	if(!$sqlite){
		...
		$statement=$sqlite->prepare('INSERT INTO passwordRecoveryRequests
		('.implode(',' , $insertCols).')
		VALUES
...
function psEditWAArenderAdmin_passwordRecoveryRequests($dat){
	...
	$resResult=RMF_TBL_WAArenderAdmin_passwordRecoveryRequests_SQLITE_RESOLVE($sqlite, $OGvalue);
	...
	array_push($bindArr, [
		...
		$statement=$sqlite->prepare('UPDATE passwordRecoveryRequests
		SET 
...
function psDropWAArenderAdmin_passwordRecoveryRequests($dat){
	...
	if(!$sqlite){
		...
		$res=RMF_TBL_WAArenderAdmin_passwordRecoveryRequests_SQLITE_RESOLVE($sqlite, $dat);
	...
	$bindArr=[[
		...
		$statement=$sqlite->prepare('DELETE FROM passwordRecoveryRequests
		WHERE pk=:pk');
		if(!$statement){
			return 'failed on prepare '.$sqlite->lastErrorMsg();
		}
...
function psGetWAArenderAdmin_passwordRecoveryRequests($dat){
	...
	$resResult=RMF_TBL_WAArenderAdmin_passwordRecoveryRequests_SQLITE_RESOLVE($sqlite, $dat);
	if($resResult !== ''){
...
function psSearchWAArenderAdmin_passwordRecoveryRequests($dat){
	...
	if(!count($whereCols)){
		...
		$statement=$sqlite->prepare('SELECT member,pk,timeAdded,token FROM passwordRecoveryRequests
