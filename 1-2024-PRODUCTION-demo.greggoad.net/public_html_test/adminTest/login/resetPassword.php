...
$sqlite->query("DELETE FROM passwordRecoveryRequests WHERE (JULIANDAY(CURRENT_TIMESTAMP)- JULIANDAY(timeAdded))*24*60 > 15");
	...
	if(!$row=$result->fetchArray(SQLITE3_ASSOC)){
		header("location: resetPasswordExpired.html");
		die();
	}
	...
	$captureToken=bin2hex(random_bytes(20));
	
	
	
	$res=psAddWAArenderAdmin_passwordRecoveryCaptures([
		'member'=>['pk'=>$row['member']],
		'token'=>$token,
		'captureToken'=>$captureToken
	]);
	$sqlite->close();
	if($res !== "SUCCESS"){
		die("<h1>There was a server error, please try again later</h1>");
	}
...
<title>Password Reset | Demo Greg Goad Dot Net</title>

<meta name="description" content="Demo Greg Goad Dot Net Password Reset"/>
...
function SubmitIt(e,frm){
	...
	ElFetch(waitDiv, _el.CREATE('div','','',{},["Resetting Password"]),
	"resetPasswordAjax.php",{method:"POST", body:fd}, "text", {
		success:function(){
			...
			_el.APPEND(document.body, [
				...
				_el.CREATE('a','','',{href:'.'},[_el.CREATE('span','','',{},["Click Here to Log In"])])
			]);
		},
		fail:function(v){
			waitDiv.className="error";
			if(v === "EXPIRED"){
				location.href="resetPasswordExpired.html";
			}
		}
	},{
		form:frm
...
<body>
  <h1>Demo Greg Goad Dot Net Password Reset</h1>

  <form id="recoveryForm" onsubmit="SubmitIt(event,this)">
    <label for="password">Password:</label>
    <input  id="password" name="password" required>
    <label for="passwordMatch">Re Enter Password:</label>
    <input  id="passwordMatch" name="passwordMatch" required>
	<input type="hidden" name="token" value="<?php echo htmlentities($token);?>">
	<input type="hidden" name="captureToken" value="<?php echo htmlentities($captureToken);?>">
	<br><br>
    <button type="submit">Submit</button>
  </form>
  <h2>This interface will Exprire it 5 Minutes</h2>
  <p>If you refresh or navigate away, <br> you'll have to start over.</p>
  <div id="waitDiv"></div>
</body>
</html>